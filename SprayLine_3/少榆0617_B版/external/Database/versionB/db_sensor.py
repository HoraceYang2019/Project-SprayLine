...
def insert_sensor_readings_batch(conn, readings: list[dict]) -> None:
    ...
    readings = [
        {
            ...
            "speed_mm_s": 300.0,
            "data_quality_flag": "normal",   # normal / interpolated / outlier（outlier 是否保留待確認）
        },
        ...
    ]
    ...
    data_quality_flag 預設為 'normal'；補值資料可標為 'interpolated'。
    'outlier' 目前仍在 Database/versionB schema 中允許，但最終是否保留待余宇承確認。
    """
    if not readings:
        return
