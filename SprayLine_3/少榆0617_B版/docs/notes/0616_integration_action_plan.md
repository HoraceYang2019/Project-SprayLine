# 0616 Integration Action Plan（0616ver_4）
...
因此 0616ver_4 仍以 `rules/sensor_thresholds.json` 作為正式判斷來源，
DB table 暫視為備用或 DB 端保留內容。

`rules/sensor_event_mapping.json` 目前是少榆端草案 mapping，目的只是讓 0616ver_4 能把 alert / status payload 補完整。`cause_id` / `response_id` 是否就是正式欄位語意，仍待余宇承確認。
