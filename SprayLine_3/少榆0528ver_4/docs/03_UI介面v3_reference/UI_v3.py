...
<body>
    ...
    <script>
            ...
            stationData.forEach(station => {
                ...
                card.innerHTML = `
                    ...
                    <div class="action-row">
                        <button class="toggle-btn ${hasBad ? "danger" : issues.length > 0 ? "warn" : "info"}" onclick="toggleAllIssues('${station.id}')">
                            ${issues.length > 0 ? "查看異常原因與改善 FaultDetail" : "零件正常 NoFault"}
                        </button>

                        <button class="toggle-btn" onclick="toggleProcessPanel('${station.id}')">
                            製程參數 ProcessParameters
                        </button>

                        <button class="toggle-btn" onclick="toggleImagePanel('${station.id}')">
                            噴幅影像 SprayWidthImage
                        </button>
                    </div>
        ...
        function restoreOpenedPanels() {
            ...
            if (openedFault) {
                if (openedFault.componentKey === "ALL") {
                    renderAllIssues(openedFault.stationId);
                } else {
                    renderComponentIssue(openedFault.stationId, openedFault.componentKey);
                }
            }
        }
        ...
        function toggleAllIssues(stationId) {
            if (openedFault && openedFault.stationId === stationId && openedFault.componentKey === "ALL") {
        ...
        function closeFaultDetail(stationId) {
            ...
            if (openedFault && openedFault.stationId === stationId) {
                openedFault = null;
            }
        }
        ...
        function renderComponentIssue(stationId, componentKey) {
            ...
            panel.className = component.level === "ok" ? "fault-detail-panel normal-detail" : "fault-detail-panel";
            panel.style.display = "block";
            panel.innerHTML = `
                <h4>${component.name} ${component.en}｜${statusText(component.level)}</h4>
                <div class="fault-item">
                    <div><span class="fault-label">目前數值：</span>${component.value}</div>
                    <div><span class="fault-label">問題說明：</span>${component.level === "ok" ? "目前狀態穩定，暫時不需要處理。" : component.issue}</div>
                    <div><span class="fault-label">可能原因：</span>${component.level === "ok" ? "無明顯異常。" : component.reason}</div>
                    <div><span class="fault-label">處理建議：</span>${component.level === "ok" ? "維持定期檢查即可。" : component.solution}</div>
                </div>
        ...
        function renderAllIssues(stationId) {
            ...
            panel.className = "fault-detail-panel";
            panel.style.display = "block";
            panel.innerHTML = `
                <h4>${data.station.name}｜異常原因與改善建議</h4>
                ${issues.map(component => `
                    <div class="fault-item">
                        <div><span class="fault-label">零件：</span>${component.name} ${component.en}｜${statusText(component.level)}</div>
                        <div><span class="fault-label">目前數值：</span>${component.value}</div>
                        <div><span class="fault-label">問題說明：</span>${component.issue}</div>
                        <div><span class="fault-label">可能原因：</span>${component.reason}</div>
                        <div><span class="fault-label">處理建議：</span>${component.solution}</div>
                    </div>
                `).join("")}
                <button class="toggle-btn" onclick="closeFaultDetail('${stationId}')">收起 Close</button>
            `;
    ...
    if not HTML_FILE.exists():
        return HTMLResponse(
            content=(
                ...
                "<p>請先執行 spraying_dashboard_updated_fixed_v3.py，"
