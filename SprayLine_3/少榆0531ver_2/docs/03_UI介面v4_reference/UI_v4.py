...
<body>
    ...
    <script>
        ...
        function buildStationData() {
            ...
            return baseStations.map((s, index) => {
                ...
                return {
                    ...
                    utilization: clamp(Math.round(s.baseUtilization - Math.max(0, factor) * 2), 0, 100),
                    cycle,
                    line_id: s.line_id || s.lineId,
                    metrics: {
                        availability_pct: availability,
                        clog_rate_pct: clog,
                        pressure_bar: Number(pressure.toFixed(1)),
                        flow_rate_ml_min: null,
                        maintainability_pct: maintainability,
                        quality_score_pct: quality,
                        risk_text: riskText,
                        spray_width_mm: sprayWidth
                    },
                    process_parameters: {
                        recipe_name: s.recipe,
                        temperature_c: Number(temp.toFixed(1)),
                        utilization_pct: clamp(Math.round(s.baseUtilization - Math.max(0, factor) * 2), 0, 100),
                    ...
                    component_metrics: {
                        nozzle: {
                            pressure_bar: Number(pressure.toFixed(1)),
                            flow_rate_ml_min: null,
                            nozzle_clog_rate_pct: clog,
                            nozzle_maintainability_pct: maintainability
                        },
                        filter_mesh: {
                            flow_loss_pct: clog,
                            filter_clog_rate_pct: clog,
                            filter_maintainability_pct: maintainability
                        },
                        spray_width: {
                            spray_width_mm: sprayWidth,
                            width_error_mm: sprayWidth - 120,
                            width_error_pct: null,
                            coverage_score_pct: quality,
                            target_min_mm: 110,
                            target_max_mm: 130
                        }
                    },
                    spray_width_image: {
                        ...
                        status: null,
                        image_ref: null,
                        note: "UI_v4 simulated data; not connected to schema/service output yet."
                    }
                };
            });
        }
        ...
        function componentOverview(station) {
            ...
            latestDiagnosis[station.lineId] = {
                station,
                components,
                ...
                <div class="component-wrap">
                    ...
                    <div class="component-overview">
                        ${components.map(component => `
                            <div class="component-mini ${component.level}" onclick="openComponentDetail('${station.lineId}', '${component.key}')">
            ...
            stationData.forEach(station => {
                ...
                card.innerHTML = `
                    ...
                    <div class="action-row">
                        <button id="fault-btn-${station.lineId}" class="toggle-btn ${hasBad ? "danger" : issues.length > 0 ? "warn" : "info"}" onclick="toggleAllIssues('${station.lineId}')">
                            ${issues.length > 0 ? "查看異常原因與改善 FaultDetail" : "零件正常 NoFault"}
                        </button>

                        <button id="process-btn-${station.lineId}" class="toggle-btn" onclick="toggleProcessPanel('${station.lineId}')">
                            製程參數 ProcessParameters
                        </button>

                        <button id="image-btn-${station.lineId}" class="toggle-btn" onclick="toggleImagePanel('${station.lineId}')">
                            噴幅影像 SprayWidthImage
                        </button>
                    </div>

                    <div class="fault-detail-panel" id="fault-${station.lineId}"></div>

                    <div class="process-panel" id="process-${station.lineId}">
                    ...
                    <div class="spray-image-box" id="image-${station.lineId}">
        ...
        function restoreOpenedPanels() {
            openedProcessPanels.forEach(lineId => {
                const panel = document.getElementById(`process-${lineId}`);
                const btn = document.getElementById(`process-btn-${lineId}`);
            ...
            openedImagePanels.forEach(lineId => {
                const panel = document.getElementById(`image-${lineId}`);
                const btn = document.getElementById(`image-btn-${lineId}`);
            ...
            openedFaultPanels.forEach((componentKey, lineId) => {
                if (componentKey === "ALL") {
                    renderAllIssues(lineId);
                } else {
                    renderComponentIssue(lineId, componentKey);
                }
            });
        }

        function toggleProcessPanel(lineId) {
            const panel = document.getElementById(`process-${lineId}`);
            const btn = document.getElementById(`process-btn-${lineId}`);
            ...
            if (panel.style.display === "block") {
                ...
                openedProcessPanels.delete(lineId);
                ...
                panel.style.display = "block";
                openedProcessPanels.add(lineId);
        ...
        function toggleImagePanel(lineId) {
            const panel = document.getElementById(`image-${lineId}`);
            const btn = document.getElementById(`image-btn-${lineId}`);
            ...
            if (panel.style.display === "block") {
                panel.style.display = "none";
                openedImagePanels.delete(lineId);
        ...
        function openComponentDetail(lineId, componentKey) {
            openedFaultPanels.set(lineId, componentKey);
            renderComponentIssue(lineId, componentKey);
        }

        function toggleAllIssues(lineId) {
            if (openedFaultPanels.get(lineId) === "ALL") {
                closeFaultDetail(lineId);
                return;
            }

            openedFaultPanels.set(lineId, "ALL");
            renderAllIssues(lineId);
        }

        function closeFaultDetail(lineId) {
            const panel = document.getElementById(`fault-${lineId}`);
            ...
            const btn = document.getElementById(`fault-btn-${lineId}`);
            if (btn) {
                btn.classList.remove("active-fault");
            }

            openedFaultPanels.delete(lineId);
        }

        function renderComponentIssue(lineId, componentKey) {
            const data = latestDiagnosis[lineId];
            const panel = document.getElementById(`fault-${lineId}`);
            ...
            if (component.level === "ok") {
                panel.innerHTML = `
                    ...
                    <button class="toggle-btn" onclick="closeFaultDetail('${lineId}')">收起 Close</button>
                `;
                return;
            }
        ...
        function renderAllIssues(lineId) {
            const data = latestDiagnosis[lineId];
            const panel = document.getElementById(`fault-${lineId}`);
