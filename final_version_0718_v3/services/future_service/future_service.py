...
PREDICTED_OK_RATE_SEMANTICS = "legacy_process_quality_proxy_not_measured_yield"
ESTIMATED_DEFECT_RATE_SEMANTICS = "estimated_process_abnormal_risk_not_measured_defect_rate"
...
def compute_risk_level(predicted_ok_rate: Optional[float], predicted_ng_count: Optional[int]) -> Optional[str]:
    """Compute process risk from a score proxy and abnormal-count estimate.

    ``predicted_ok_rate`` is the legacy parameter name. In linear_trend_v1 it
    receives the derived process-quality score, not a measured product yield.

    Current project rule:
    - high: process-quality score < 84 or estimated abnormal count >= 35
    - medium: process-quality score < 90 or estimated abnormal count >= 20
    - low: otherwise
    """
...
def build_future_prediction_payload(
    ...
    estimated_defect_rate_pct: Optional[float] = None,
    quality_score_semantics: str = QUALITY_SCORE_SEMANTICS,
    rule_evaluations: Optional[Dict[str, Any]] = None,
    ...
    """Build a Future payload with legacy fields and Ontology decisions."""
    normalized_prediction_time = _parse_datetime(prediction_time)
    if normalized_prediction_time is None:
        raise ValueError("prediction_time must be a valid ISO-8601 datetime")
    prediction_time = normalized_prediction_time.astimezone(timezone.utc).isoformat()
    predicted_metrics = dict(predicted_metrics or {})
    predicted_components = dict(predicted_components or {})
    rule_evaluations = dict(rule_evaluations or {})
    ...
    cause_ids = unique_strings(cause_ids)
    response_ids = unique_strings(response_ids)
    rule_sources = unique_strings(rule_sources)
    rule_engines = unique_strings(
        result.get("rule_engine")
        for result in rule_evaluations.values()
        if isinstance(result, dict)
    )
    if estimated_defect_rate_pct is None:
        estimated_defect_rate_pct = _to_float(
            predicted_metrics.get("estimated_defect_rate_pct")
        )
    risk_score = quality_score if quality_score is not None else predicted_ok_rate
    payload = {
        ...
        "quality_score_semantics": quality_score_semantics,
        "predicted_ok_rate_semantics": PREDICTED_OK_RATE_SEMANTICS,
        "estimated_defect_rate_pct": estimated_defect_rate_pct,
        "estimated_defect_rate_semantics": ESTIMATED_DEFECT_RATE_SEMANTICS,
        "risk_level": compute_risk_level(risk_score, predicted_ng_count),
