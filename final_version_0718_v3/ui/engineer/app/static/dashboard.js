console.log("UI_0718_v2 dashboard.js loaded: DB-time aligned + batch performance fixes");
...
const trendAbortControllers = new Map();
...
async function showComponentDetail(stationId, componentKey, restoring = false) {
    ...
    try {
        ...
        // 0718_v2: keep the station card and detail panel on the same dashboard
        // snapshot. A click must not overwrite the top card with a newer DB row.
...
async function loadTrend(stationId, componentKey) {
    ...
    try {
        const previousController = trendAbortControllers.get(requestKey);
        if (previousController) previousController.abort();
        const controller = new AbortController();
        trendAbortControllers.set(requestKey, controller);
        const data = await fetchJson(`/api/trend-data?station_id=${encodeURIComponent(stationId)}&component_key=${encodeURIComponent(componentKey)}&mode=${encodeURIComponent(currentMode)}&slider_value=${encodeURIComponent(selectedSlider)}&snapshot_seed=${encodeURIComponent(selectedSeed)}`, { signal: controller.signal });
        ...
        if (note) {
            note.textContent = data.available === false
                ? `${data.message || "此時間點／批次尚無此零件資料"}｜未使用 UI 補值`
                : `已呼叫：${data.service_api?.route || "component-detail"}｜趨勢方式：${data.service_api?.method || "time_series"}｜目前 slider：${selectedSlider}｜圈選點：${selected.display_label || "目前卡片"}／${selected.formatted_value || "--"}｜門檻來源：${source}`;
        }
    } catch (error) {
        ...
        if (trendAbortControllers.get(requestKey)?.signal.aborted === false) {
            trendAbortControllers.delete(requestKey);
        }
    }
}
...
async function refreshDashboard() {
    ...
    try {
        ...
        const payload = await fetchJson(`/api/dashboard-data?mode=${currentMode}&slider_value=${selectedSlider}`, { signal: dashboardAbortController.signal });
...
function updateSliderPreview() {
    ...
    autoUpdate = value === 0;
    const label = currentMode === "batch"
        ? (value === 0 ? "目前批次" : value < 0 ? `過去第${Math.abs(value)}批` : `未來第${value}批`)
        : (value === 0 ? "現在" : value < 0 ? `過去${Math.abs(value)}小時` : `未來${value * 0.5}小時`);
    document.getElementById("selectedTimeText").textContent = `目前時間點：${label}`;
}
