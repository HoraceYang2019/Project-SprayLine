...
class DashboardService:
    ...
    def dashboard_payload(self, slider_value: float = 0, mode: str = "time") -> dict[str, Any]:
        ...
        # V29 display-time normalization:
        # The database simulation may contain timestamps from future years such as 2032.
        # Keep the original PostgreSQL timestamp in data_timestamp for traceability,
        # but show the viewer/slider time as the station Timestamp so the UI stays
        # consistent with the classroom/demo current time.
        display_time_iso = selected_time.isoformat()
        for station in station_cards:
            station["data_timestamp"] = station.get("timestamp")
            station["timestamp"] = display_time_iso
            process_params = station.get("process_parameters")
            if isinstance(process_params, dict):
                process_params["data_timestamp"] = process_params.get("timestamp")
                process_params["timestamp"] = display_time_iso
        ...
        return {
            ...
            "viewer_state": {
                ...
                "display_label": self._display_label(mode, slider_value),
                # V21: selected_time is for UI display and should follow the real
                # viewer clock/slider (2026 on the classroom machine).
                # data_anchor_time keeps the PostgreSQL data clock (for example 2029)
                # used internally by the Service API to query sensor_1min.
                "selected_time": selected_time.isoformat(),
    ...
    def trend_payload(
        ...
        selected_time = response_anchor or self._selected_time(mode, slider_value)
        ...
        source_note = (
            "趨勢圖以少榆 component-detail function 多時間點取樣產生；"
            "每一點皆由 Service API 依 slider_value 查詢/預測，藍色圈選點會跟時間滑桿同步。"
        )
        ...
        # V21 keeps the trend data timestamps as PostgreSQL timestamps, but exposes
        # the UI selected time separately for labels that should follow the real clock.
        ui_display_time = self._selected_time(mode, slider_value)
