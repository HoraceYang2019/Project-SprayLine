...
# In final_version, /app/services and /app/database are supplied by PYTHONPATH.
...
def _http_status_for_exception(exc: BaseException) -> tuple[int, str]:
    """Map common validation/DB failures to an HTTP status and stable code."""
    class_names = {cls.__name__ for cls in exc.__class__.mro()}
    ...
    if isinstance(exc, ConnectionError) or class_names.intersection(
    ...
    if class_names.intersection(
        ...
        return 409, "database_conflict"
    if class_names.intersection(
        {"ProgrammingError", "UndefinedColumn", "UndefinedTable", "SyntaxError"}
    ):
        return 500, "database_schema_error"
    return 500, "internal_error"


def _error(stage: str, exc: BaseException) -> Dict[str, Any]:
    http_status, error_code = _http_status_for_exception(exc)
    LOGGER.exception("Service orchestration failed at %s", stage)
    return _json_safe({
        "success": False,
        "stage": stage,
        ...
        "note": "See API container logs for internal details.",
    })
...
def get_service_orchestration_status() -> Dict[str, Any]:
    ...
    checks = {
        "IntegratedSprayLineService": "integrated_service.sprayline_integrated_service",
        "FutureServiceFunctions": "future_service.future_service",
        "MonitoringWorker": "monitoring_worker.monitoring_worker",
        "TroubleshootingService": "troubleshooting_service.troubleshooting_service",
        "DatabaseVersionBAdapter": "integration_adapter.database_versionb_adapter",
    }
    ...
    try:
        from integration_adapter.database_versionb_adapter import get_adapter_status
...
def build_integrated_request(
    ...
    try:
        from integrated_service.sprayline_integrated_service import DEFAULT_REQUESTED_METRICS
    except Exception:
...
def build_future_prediction_payload_for_api(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        from future_service.future_service import build_future_prediction_payload
        payload = build_future_prediction_payload(
            batch_id=data.get("batch_id", "BATCH_UNKNOWN"),
            station_id=data.get("station_id"),
            prediction_time=data.get("prediction_time") or datetime.now(timezone.utc).isoformat(),
            predicted_ok_rate=data.get("predicted_ok_rate"),
            predicted_ng_count=data.get("predicted_ng_count"),
            quality_score=data.get("quality_score"),
            prediction_method=data.get("prediction_method", "deterministic_formula"),
            ...
            predicted_components=data.get("predicted_components"),
            estimated_defect_rate_pct=data.get("estimated_defect_rate_pct"),
            quality_score_semantics=data.get(
            ...
            horizon_minutes=data.get("horizon_minutes"),
            input_window=data.get("input_window"),
            confidence=data.get("confidence"),
            metric_diagnostics=data.get("metric_diagnostics"),
        )
        return _ok({
...
def save_future_prediction_payload_for_api(data: Dict[str, Any], commit: bool = True) -> Dict[str, Any]:
    ...
    try:
        from integration_adapter.database_versionb_adapter import get_connection
...
def get_troubleshooting_matrix_for_api(asset_type: Optional[str] = None, state: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        from integration_adapter.database_versionb_adapter import get_connection
        from troubleshooting_service.troubleshooting_service import get_troubleshooting_matrix
        conn = get_connection()
        try:
...
def get_state_recommendations_for_api(state: str, station: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        from integration_adapter.database_versionb_adapter import get_connection
        from troubleshooting_service.troubleshooting_service import get_state_recommendations
        conn = get_connection()
        try:
