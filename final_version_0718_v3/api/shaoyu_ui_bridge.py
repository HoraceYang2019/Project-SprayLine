...
def _metric_dict_from_station(station: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if future:
        process_quality_score = metrics.get("quality_score_pct")
        if process_quality_score is None:
            process_quality_score = metrics.get("quality_score")
        if process_quality_score is None:
            process_quality_score = metrics.get("predicted_ok_rate")
        if process_quality_score is not None:
            metrics["quality_score_pct"] = process_quality_score
            metrics["quality_score"] = process_quality_score
            if metrics.get("estimated_defect_rate_pct") is None:
                metrics["estimated_defect_rate_pct"] = round(
                    max(0.0, 100.0 - float(process_quality_score)),
                    2,
                )
            if metrics.get("qc_pct") is None:
