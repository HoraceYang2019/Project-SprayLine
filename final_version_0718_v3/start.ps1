...
<#
.SYNOPSIS
    SprayLine final_version_0718_v3 - Docker startup script
...
function Wait-DbMigration([int]$maxSec = 90) {
    Write-Step "Waiting for db-migrate to finish (max ${maxSec}s)..."
    $elapsed = 0
    while ($elapsed -lt $maxSec) {
        $psOut = docker compose ps -a db-migrate 2>$null | Out-String
        if ($psOut -match "Exited \(0\)") {
            Write-OK "db-migrate completed"
            return $true
        }
        if (($psOut -match "Exited") -and ($psOut -notmatch "Exited \(0\)")) {
            Write-Fail "db-migrate failed. Run: docker compose logs db-migrate"
            return $false
        }
        Start-Sleep -Seconds 3
        $elapsed += 3
    ...
    Write-Fail "db-migrate timed out. Run: docker compose logs db-migrate"
    return $false
}

function Invoke-Start {
    Write-Header "Start 0718_v3 Services (existing 0627 database)"

    Write-Info "Step 1/3: Starting external PostgreSQL volume (no db-setup)"
    ...
    Write-Warn "0718_v3 is configured for final_version_0627_sprayline_pgdata."
    Write-Warn "Normal start will NOT run db-setup and will NOT start dataprocess."

    Write-Info "Step 2/3: Apply non-destructive backend DB migrations"
    docker compose up -d --build db-migrate
    if ($LASTEXITCODE -ne 0) {
        Write-Fail "DB migration failed. Run: docker compose logs db-migrate"
        exit 1
    }
    $migrationOk = Wait-DbMigration -maxSec 90
    if (-not $migrationOk) { exit 1 }

    Write-Info "Step 3/3: Build and start api + frontend + engineer"
...
function Invoke-Restart {
    ...
    docker compose up -d --build db-migrate
    if ($LASTEXITCODE -ne 0) {
        Write-Fail "DB migration failed. API was not restarted."
        exit 1
    }
    $migrationOk = Wait-DbMigration -maxSec 90
    if (-not $migrationOk) { exit 1 }
...
switch ($Mode) {
    ...
    "reset"   { Invoke-Validate; Write-Fail "Reset is disabled in 0718_v3 to protect final_version_0627_sprayline_pgdata."; exit 1 }
