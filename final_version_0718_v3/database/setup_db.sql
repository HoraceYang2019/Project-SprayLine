...
CREATE TABLE batch_station_status (
    ...
    quality_score_pct         REAL         CHECK (quality_score_pct BETWEEN 0 AND 100),
    qc_pct                    REAL         CHECK (qc_pct BETWEEN 0 AND 100),
    estimated_defect_rate_pct REAL         CHECK (estimated_defect_rate_pct BETWEEN 0 AND 100),
    estimated_film_thickness_um REAL,
...
CREATE TABLE future_prediction_result (
    ...
    quality_score        REAL,
    estimated_defect_rate_pct REAL CHECK (estimated_defect_rate_pct BETWEEN 0 AND 100),
    quality_score_semantics VARCHAR(80) NOT NULL
        DEFAULT 'process_quality_score_not_measured_yield',
...
CREATE UNIQUE INDEX uq_future_prediction_idempotency
