...
讓 FutureService / MonitoringWorker 可以直接 import 提供的 Python DB functions。
...
def get_latest_sensor_1min(conn, station_id: str):
    module = import_database_module("db_sensor")
    return module.get_latest_sensor_1min(conn, station_id)


def get_running_batches(conn) -> list[dict]:
    module = import_database_module("db_batch")
    return module.get_running_batches(conn)


def get_latest_batches(conn, limit: int = 10) -> list[dict]:
    module = import_database_module("db_batch")
    return module.get_latest_batches(conn, limit=limit)
...
def get_unacknowledged_alerts(conn, station_id: str | None = None, limit: int = 50) -> list[dict]:
    module = import_database_module("db_alert")
    return module.get_unacknowledged_alerts(conn, station_id=station_id, limit=limit)
