...
class WebservicesClient:
    ...
    def _service_slider_value(mode: str, slider_value: float) -> float:
        """Convert the engineer UI axis into the Service API unit.

        The Service API build_time_window() accepts minutes:
        - TimeMode past: one UI step is one hour.
        - TimeMode future: one UI step is half an hour.
        - BatchMode: the value is a batch offset and must remain unchanged.
        """
    ...
    def _common_payload(self, *, mode: str, slider_value: float, random_seed: int, request_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "ui_slider_unit": "hour_index_converted_to_minutes" if mode != "batch" else "batch_index",
