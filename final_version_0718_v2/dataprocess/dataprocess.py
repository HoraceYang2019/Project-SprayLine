...
def clean_data(station, reading, last_valid):
    if reading['data_quality_flag'] == 'interpolated':
        if last_valid[station] is not None:
            ...
            reading['film_thickness_um'] = last_valid[station].get('film_thickness_um')
    ...
    while True:
        ...
        if sec == 50:
            ...
            for station, config in STATION_CONFIG.items():
                ...
                # A 狀態：維修中（極端故障值，會被標記為 interpolated 並清洗成平線）
                ...
                # 0620ver_1：補 Quality 需要的 film_thickness_um，避免 dataprocess 持續寫入後 Quality 變成 NULL。
                # 膜厚先以 15um 為中心，受塗料流量、空壓、噴幅偏差與雜訊微幅影響；正式公式可再接伯儒演算法。
                air_pressure = round(np.random.normal(3.2, 0.05), 2)
                spray_width_value = round(np.random.normal(spray_width, 0.5), 2)
                spray_width_error = abs(spray_width_value - config["spray_width_base"])
                film_thickness = 15.0
                film_thickness += (paint_flow - 115.0) * 0.015
                film_thickness += (air_pressure - 3.2) * 0.30
                film_thickness -= spray_width_error * 0.015
                film_thickness += np.random.normal(0, 0.08)
                ...
                reading = {
                    ...
                    "spray_width_mm": spray_width_value,
