...
#"clearcoat.engineer@example.com","primer.engineer@example.com", "topcoat.engineer@example.com",
...
MANAGER_DAILY_TARGET_PCS = 20000
...
def build_manager_dashboard_payload(api_bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    line_quality = round(_average(predicted_ok_rates), 1)
    line_utilization = round(_average(utilizations), 1)
    avg_cycle_time = round(_average(cycles), 1)
    accuracy_items = list((api_bundle.get("predictionAccuracy") or {}).values())
    accuracy_pct = _number(accuracy_items[0].get("accuracy_pct") if accuracy_items else 88.0, 88.0)
    prediction_error_pts = round(max(0.6, (100.0 - accuracy_pct) / 10.0), 1)
    actual_ok_rate = round(min(99.9, line_quality + min(1.5, prediction_error_pts * 0.4)), 1)
    actual_ng_pcs = round(max(0, produced_total * (100.0 - actual_ok_rate) / 100.0))
    predicted_ng_total = predicted_ng_total or round(max(0, produced_total * (100.0 - line_quality) / 100.0))

    previous_efficiency = round(min(99.0, max(line_utilization + 4.0, _average([
        station["baseline"]["utilization_pct"] for station in station_telemetry
    ]) or line_utilization)), 1)
    previous_ok_rate = round(min(99.0, line_quality + 2.2), 1)
    previous_produced = max(produced_total, planned_total)
    previous_actual_ng = round(max(0, previous_produced * (100.0 - previous_ok_rate) / 100.0))
    yesterday_efficiency = round(max(0.0, previous_efficiency - 1.5), 1)
    month_efficiency = round(_average([line_utilization, previous_efficiency]), 1)
    last_month_efficiency = round(min(99.0, previous_efficiency + 1.8), 1)

    quality_validation_date = selected_date or (selection_anchor - timedelta(days=1)).date().isoformat()
    future_efficiency = round(max(0.0, line_utilization - (6.0 + warning_count * 2.0)), 1)
    future_ok_rate = round(max(0.0, line_quality - (2.4 + warning_count * 0.8)), 1)
    future_lost_pcs = max(0, round(planned_total * 0.08 + warning_count * 40))
    future_extra_ng = max(0, round(predicted_ng_total * 0.35 + warning_count * 25))
