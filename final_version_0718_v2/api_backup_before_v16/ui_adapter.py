...
def BuildUiStationCard(station: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "air_pressure_bar": RoundValue(metrics.get("air_pressure_bar")),
        "flow_rate_ml_min": RoundValue(metrics.get("flow_rate_ml_min")),
        "paint_flow_ml_min": RoundValue(metrics.get("paint_flow_ml_min")),
        ...
        "monitoring": station.get("d_payloads", {}).get("monitoring", {}),
        "event_rule": station.get("d_payloads", {}).get("event_rule", {}),
        "future_prediction": station.get("d_payloads", {}).get("future_prediction"),
        "troubleshooting": station.get("d_payloads", {}).get("troubleshooting", []),
    }
...
def BuildUiSummaryOutput(core_output: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "d_integration": core_output.get("d_integration", {}),
        "stations": [
...
def BuildUiComponentDetailOutput(
    ...
    component_state = next(
        (item for item in component_overview if item.get("component_name") == component_name),
        None
    )
    troubleshooting = [
        item for item in station.get("d_payloads", {}).get("troubleshooting", [])
        if item.get("component_name") == component_name
    ]
