(function () {
    ...
    window.renderTrendChart = function renderTrendChart(container, payload) {
        ...
        const currentIndex = points.findIndex(point => point.time_type === "current");
        if (currentIndex >= 0) {
        ...
        points.forEach((point, index) => {
            const dot = svgEl("circle", { cx: xAt(index), cy: yAt(point.value), r: point.time_type === "current" ? 5 : 3.3, fill: COLORS[point.level] || "#607d8b", stroke: "white", "stroke-width": 1.5 });
            const title = svgEl("title", {}, `${formatProductionTime(point.timestamp)}｜${point.value} ${payload.unit || ""}｜${point.level === "bad" ? "異常" : point.level === "warn" ? "注意" : "正常"}`);
        ...
        [...new Set(xLabels)].forEach(index => {
            const label = points[index].time_type === "current" ? "現在" : formatProductionTimeShort(points[index].timestamp);
            svg.appendChild(svgEl("text", { x: xAt(index), y: margin.top + plotH + 22, "text-anchor": "middle", fill: "#607d8b", "font-size": 11 }, label));
        });

        svg.appendChild(svgEl("text", { x: width / 2, y: height - 7, "text-anchor": "middle", fill: "#455a64", "font-size": 12, "font-weight": 700 }, "過去 6 小時　→　現在　→　未來 2 小時"));
