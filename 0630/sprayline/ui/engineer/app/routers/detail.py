...
def get_component_detail(
    ...
    return dashboard_service.component_detail(station_id, component_key, slider_value=slider_value, mode=mode, anchor_batch_id=anchor_batch_id)
...
def get_station_detail(
    ...
    if station_id not in STATIONS:
        raise HTTPException(status_code=404, detail="unknown station_id")
    return dashboard_service.station_detail(station_id, slider_value=slider_value, mode=mode, anchor_batch_id=anchor_batch_id)
