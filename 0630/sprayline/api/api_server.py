...
from services.integrated_service.quality_score_common import quality_score_from_formal_row
...
def _build_manager_trends_for_station(
    ...
    for hour in sorted(hourly_rows.keys()):
        ...
        quality_series.append({
            "hour": hour,
            "quality_score_pct": _quality_score_from_point(pseudo_point),
        })
...
def _build_manager_bundle_from_database(
    ...
    for line_id, station_id in LINE_TO_STATION.items():
        ...
        quality_score = _quality_score_from_point(signal_metrics)
        ...
        clog_rate_pct = round(min(100.0, filter_bar / _FILTER_FAULT_BAR * 100.0), 1)
