...
COMPONENT_METRICS = {
    "quality_module": ["quality_score_pct", "estimated_defect_rate_pct", "qc_pct", "estimated_film_thickness_um", "predicted_ok_rate"],
    "quality": ["quality_score_pct", "estimated_defect_rate_pct", "qc_pct", "estimated_film_thickness_um", "predicted_ok_rate"],
...
def _metric_dict_from_station(station: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # In future mode, future_prediction_payload should be the visible value
    # for quality/risk.  Otherwise Future +1 / +2 still looks identical to
    # the anchor snapshot even though prediction_time changed.
    source_order = (future, snapshot, station_metrics, summary) if future else (snapshot, station_metrics, summary, future)
