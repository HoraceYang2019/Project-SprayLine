...
def infer_state(
    metric: str,
    value: float,
    ttl_path: str | Path = "ontology/sprayline_threshold.ttl",
) -> dict[str, Any]:
    ...
    # 2. 警告區間
        ...
        _between(
            value,
            rule.warning_low_min,
            rule.warning_low_max,
            high_inclusive=False,
        )
        or _between(
            value,
            rule.warning_high_min,
            rule.warning_high_max,
            low_inclusive=False,
        )
    ):
        state = "warning"
    ...
    # 3. 故障區間
        ...
        (
            rule.fault_low_max is not None
            and value <= rule.fault_low_max
        )
        or (
            rule.fault_high_min is not None
            and value > rule.fault_high_min
        )
    ):
        state = "fault"
        level = "fault"
    ...
    # 4. 規則沒有定義的區間，不直接視為故障
        ...
        state = "unknown"
        level = "unknown"
        issue = "該值未落在已配置的正常、警告或故障區間。"

    is_abnormal = state in {"warning", "fault"}
    ...
    return {
        ...
        "level": level,
        "issue": issue,
        "cause_id": rule.cause_id if is_abnormal else None,
        "response_ids": rule.response_ids if is_abnormal else [],
        "threshold": {
