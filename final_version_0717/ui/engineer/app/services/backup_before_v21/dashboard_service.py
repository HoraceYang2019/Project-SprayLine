...
class DashboardService:
    ...
    def _build_station(
        ...
        *,
        ...
        station_timestamp = (
            ...
            or (station_detail.get("current_snapshot") or {}).get("ts")
    ...
    def dashboard_payload(self, slider_value: float = 0, mode: str = "time") -> dict[str, Any]:
        ...
        component_route_expected = len(STATIONS) * len(COMPONENTS)
        ...
        for ui_id in STATIONS:
            ...
            try:
                ...
                component_errors_for_station: dict[str, str] = {}
                for spec in COMPONENTS:
                    try:
                        ...
                        component_route_ok += 1
                    except Exception as comp_exc:
                        component_errors_for_station[spec["key"]] = str(comp_exc)
                station_card = self._build_station(
                    summary_station,
                    detail_response,
                    component_responses=component_responses,
                )
                if component_errors_for_station:
                    station_card["component_errors"] = component_errors_for_station
                    component_errors[ui_id] = component_errors_for_station
        ...
        for station in station_cards:
            anchor_time = self._parse_service_time(station.get("timestamp"))
            if anchor_time is not None:
                break
        if anchor_time is None:
            anchor_time = selected_time
        ...
        return {
            ...
            "viewer_state": {
                ...
                "display_label": self._display_label(mode, slider_value),
                "selected_time": anchor_time.isoformat(),
