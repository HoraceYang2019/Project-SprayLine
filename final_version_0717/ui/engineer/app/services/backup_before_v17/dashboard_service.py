...
class DashboardService:
    ...
    def _selected_time(mode: str, slider_value: float) -> datetime:
        ...
        if mode == "batch":
            return now + timedelta(minutes=float(slider_value) * 18)
    ...
    @staticmethod
    def _component_data_from_response(response: dict[str, Any]) -> dict[str, Any]:
    ...
    def _build_component(
        ...
        *,
        ...
        if component_response is not None:
            ...
            data = component_metrics.get(service_name) or {}
        ...
        state = str(state_item.get("state") or "").lower()
        source_sensor: str | None = None
        if state not in {"normal", "warning", "fault"}:
            state, source_sensor = self._component_state_from_data(data, spec)
        else:
            # Use the most severe triggered sensor for the explanatory label.
            ...
            if source_sensor is None:
                _, source_sensor = self._component_state_from_data(data, spec)
    ...
    def _build_station(self, summary_station: dict[str, Any], station_detail: dict[str, Any]) -> dict[str, Any]:
        ui_id = (
        ...
        components = [self._build_component(station_detail, spec) for spec in COMPONENTS]
        worst_level = max(components, key=lambda item: {"ok": 1, "warn": 2, "bad": 3}.get(item["level"], 0))["level"]

        station_state = (
            station_detail.get("state")
            or (station_detail.get("event_rule") or {}).get("station_state")
            or summary_station.get("state")
        )
        overall = self._level_to_overall(self._state_to_level(station_state)) if station_state else self._level_to_overall(worst_level)
        risk_text = station_detail.get("risk_text") or summary_station.get("risk_text")
        if not risk_text:
            risk_text = {"Alarm": "高風險", "Maintenance": "中風險", "Running": "低風險"}.get(overall, "待確認")
        ...
        return {
            ...
            "timestamp": process.get("timestamp") or station_detail.get("generated_at"),
    ...
    def dashboard_payload(self, slider_value: float = 0, mode: str = "time") -> dict[str, Any]:
        ...
        for ui_id in STATIONS:
            ...
            try:
                ...
                station_cards.append(self._build_station(summary_station, detail_response))
                station_route_ok += 1
        ...
        normal_count = remote_summary.get("normal_count")
        warning_count = remote_summary.get("warning_count")
        risk_count = remote_summary.get("predict_risk_count")
        ...
        fault_count = remote_summary.get("fault_count")
        if fault_count is not None:
            warning_count = int(warning_count or 0) + int(fault_count or 0)
        if risk_count is None:
            risk_count = sum(1 for item in station_cards if item["riskText"] != "低風險")
        ...
        return {
            ...
            "summary": {
                "total_station_count": remote_summary.get("total_station_count", len(station_cards)),
                "normal_count": normal_count or 0,
                "warning_count": warning_count or 0,
                "predict_risk_count": risk_count or 0,
            },
            ...
            "integration": {
                ...
                "route_status": {
                    ...
                    "component_detail": "called when a component or trend tab is clicked",
                },
    ...
    def station_detail(
        ...
        station = self._build_station(summary_stub, response)
    ...
    def _extract_series_points(response: dict[str, Any], metric: str) -> list[dict[str, Any]]:
        series = response.get("time_series") or {}
    ...
    def _sampled_service_trend(
        ...
        if mode == "batch":
            offsets: list[float] = [-10, -8, -6, -4, -2, 0, 2, 4, 6, 8, 10]
            if selected_slider not in offsets:
        ...
        for index, offset in enumerate(offsets):
            ...
            try:
                ...
                value = float(raw_value) if raw_value is not None else selected_value
            except Exception:
                value = selected_value

            if mode == "batch":
                timestamp = anchor + timedelta(minutes=offset * 18)
            else:
                hours = offset if offset < 0 else offset * 0.5
                timestamp = anchor + timedelta(hours=hours)
            raw_points.append({
                "timestamp": timestamp.isoformat(),
                "value": value,
                "slider_value": offset,
                "time_type": self._time_type(offset),
                "selected_snapshot": offset == float(selected_slider),
            })

        raw_values = [float(point["value"]) for point in raw_points]
        smoothed = self._moving_average(raw_values)
        selected_index = next(
            (index for index, point in enumerate(raw_points) if point["selected_snapshot"]),
            0,
        )
        shift = selected_value - smoothed[selected_index]
        digits = 3 if metric in {"filter_diff_pressure_bar", "path_error_mm", "vibration_g"} else 2
        for index, point in enumerate(raw_points):
            point["raw_service_value"] = round(raw_values[index], digits)
            point["value"] = round(smoothed[index] + shift, digits)
        raw_points[selected_index]["value"] = round(selected_value, digits)

        # Keep the displayed service-sampled curve continuous around the selected
        # card point. The raw samples are still retained in raw_service_value.
        rule = self.rules.rule(metric) or {}
        normal = rule.get("normal") or {}
        if normal.get("min") is not None and normal.get("max") is not None:
            normal_span = abs(float(normal["max"]) - float(normal["min"]))
            max_step = max(normal_span * 0.10, abs(selected_value) * 0.005)
        else:
            max_step = max(abs(selected_value) * 0.03, 0.5)

        def clamp_to_neighbor(value: float, neighbor: float) -> float:
            return max(neighbor - max_step, min(neighbor + max_step, value))

        for index in range(selected_index - 1, -1, -1):
            raw_points[index]["value"] = round(
                clamp_to_neighbor(float(raw_points[index]["value"]), float(raw_points[index + 1]["value"])),
                digits,
            )
        for index in range(selected_index + 1, len(raw_points)):
            raw_points[index]["value"] = round(
                clamp_to_neighbor(float(raw_points[index]["value"]), float(raw_points[index - 1]["value"])),
                digits,
            )
    ...
    def trend_payload(
        ...
        if selected_raw is None:
            raise ValueError(f"Service API component-detail did not return {metric}")
        selected_value = float(selected_raw)

        points = self._extract_series_points(selected_response, metric)
        response_anchor_raw = selected_response.get("data_anchor_time")
        try:
            response_anchor = datetime.fromisoformat(str(response_anchor_raw).replace("Z", "+00:00")) if response_anchor_raw else None
        except ValueError:
            response_anchor = None
        if response_anchor is not None and response_anchor.tzinfo is None:
            response_anchor = response_anchor.replace(tzinfo=timezone.utc)

        viewer = selected_response.get("viewer_state") or {}
        future_prediction = selected_response.get("future_prediction") or {}
        future_time_raw = future_prediction.get("prediction_time") or viewer.get("future_time")
        try:
            future_time = datetime.fromisoformat(str(future_time_raw).replace("Z", "+00:00")) if future_time_raw else None
        except ValueError:
            future_time = None
        if future_time is not None and future_time.tzinfo is None:
            future_time = future_time.replace(tzinfo=timezone.utc)

        selected_time = future_time if float(slider_value) > 0 and future_time else (response_anchor or self._selected_time(mode, slider_value))

        if points:
            source_method = "component-detail time_series"
            parsed_times: list[datetime | None] = []
            for point in points:
                point["selected_snapshot"] = False
                try:
                    ...
                    point_time = None
                parsed_times.append(point_time)
                if point_time is None:
                    point["time_type"] = "past"
                elif point_time < selected_time:
                    point["time_type"] = "past"
                elif point_time > selected_time:
                    point["time_type"] = "future"
                else:
                    point["time_type"] = "current"

            # Current/past snapshots normally correspond to the latest/closest DB point.
            # Future snapshots are appended at the prediction time because the stored series is the model input window.
            if float(slider_value) <= 0 and any(t is not None for t in parsed_times):
                selected_index = min(
                    (i for i, t in enumerate(parsed_times) if t is not None),
                    key=lambda i: abs((parsed_times[i] - selected_time).total_seconds()),
                )
                points[selected_index]["selected_snapshot"] = True
                points[selected_index]["value"] = selected_value
                points[selected_index]["time_type"] = self._time_type(slider_value)
                selected_time = parsed_times[selected_index] or selected_time
            else:
                points.append({
            ...
            source_note = "直接使用 Webservices component-detail 回傳的 PostgreSQL time_series。"
        else:
            ...
            source_note = (
                "目前 component-detail 未回傳 time_series，因此由 UI 後端對同一個 "
                "component-detail function 進行多時間點採樣並平滑；沒有使用 UI 本機模擬資料。"
            )
        ...
        result = {
            ...
            "selected_snapshot": {
                ...
                "timestamp": selected_time.isoformat(),
                "value": selected_value,
                "formatted_value": selected_component.get("value"),
