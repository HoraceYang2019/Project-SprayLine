...
def evaluate_sensor_payload(sensor_payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for sensor_name, value in sensor_payload.items():
        ...
        if state in {"warning", "fault"}:
            ...
            response_ids = list(classification.get("response_ids") or [])
            if cause_id:
                detected["cause_id"] = cause_id
                detected["cause"] = cause_id
            if response_ids:
                detected["response_ids"] = response_ids
                detected["primary_response_id"] = response_ids[0]
            detected.update({
